...
def main(file_arg, logs):
	log_list = []

	def write_log(string):
		with open(logs+'/ozip_decrypt.log', 'a', encoding='utf8', newline='\n') as f:
			print(string, file=f)
	...
	def keytest(data):
		for key in keys:
			...
			if (dat[0:4] == b'\x50\x4B\x03\x04'):
				log_list.append("Found correct AES key: " + key)
	...
	def decryptfile(key, rfilename):
		with open(rfilename,'rb') as rr:
			with open(rfilename+".tmp", 'wb') as wf:
				...
				rr.seek(0x1050)
				log_list.append("Decrypting " + rfilename)
	...
	def mode2(mfilename):
		temp=os.path.join(os.path.abspath(os.path.dirname(mfilename)), "temp")
		out=os.path.join(os.path.abspath(os.path.dirname(mfilename)), "out")
		with open(mfilename, 'rb') as fr:
			magic = fr.read(12)
			if magic[:2] == b"PK":
				testkey = True
				with ZipFile(file_arg, 'r') as zipObj:
					...
					if os.path.exists(out):
						rmrf(out)
					os.mkdir(out)
					log_list.append("Extracting " + file_arg)
								...
								if (magic == b"OPPOENCRYPT!"):
									...
									if testkey == True:
										with open(os.path.join(temp, "boot.img"), "rb") as rt:
											...
											key = keytest(data)
											if (key == -1):
												log_list.append("Unknown AES key, reverse key from recovery first!")
												return 1
										testkey = False
							if encrypted:
								if decryptfile2(key, rfilename, wfilename) == 1:
									return 1
							else:
								shutil.move(rfilename, wfilename)
					rmrf(temp)
					log_list.append("DONE... files decrypted to: " + out)

	log_list.append("ozipdecrypt 1.2 (c) B.Kerler 2017-2020")

	with open(file_arg, 'rb') as fr:
		magic = fr.read(12)
		if (magic == b"OPPOENCRYPT!"):
			pk = False
		elif magic[:2] == b"PK":
			...
			log_list.append("ozip has unknown magic, OPPOENCRYPT! expected!")
			write_log('\n'.join(log_list))
			return 1
		...
		if pk == False:
			...
			with ZipFile(filename, 'r') as zo:
				...
				try:
					...
					log_list.append("Detected mode 2....")
					if mode2(filename) == 1:
						write_log('\n'.join(log_list))
						return 1
					return
				if testkey:
					...
					if fname != '':
						if zo.extract(fname, outpath):
							...
							testkey = False
					if testkey == True:
						log_list.append("Unknown image, please report an issue with image name!")
						write_log('\n'.join(log_list))
						return 1

				for info in zo.infolist():
					log_list.append("Extracting " + info.filename)
				...
				log_list.append("DONE... files decrypted to: " + outpath)
				write_log('\n'.join(log_list))
				return 0
